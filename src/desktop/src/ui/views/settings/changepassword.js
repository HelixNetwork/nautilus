  ...
  changePassword = async event => {
    ...
    try {
      ...
      const accountTypes = Object.keys(accounts)
        .map(accountName =>
          accounts[accountName].meta
            ? accounts[accountName].meta.type
            : "keychain"
        )
        .filter(
          (accountType, index, accountTypes) =>
            accountTypes.indexOf(accountType) === index
        );

      for (let i = 0; i < accountTypes.length; i++) {
        await SeedStore[accountTypes[i]].updatePassword(
          passwordCurrentHash,
          passwordNewHash
        );
      }
