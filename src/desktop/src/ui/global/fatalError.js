...
const FatalError = ({ error, history }) => {
  const [errors, setErrors] = useState(
    typeof window.fatalErrors === "object" ? window.fatalErrors : []
  );
  ...
  const getContent = () => {
    if (
      typeof error === "string" &&
      error.indexOf("Unknown or unsupported transport") > -1
    ) {
      return (
        <form>
    ...
    if (
      ...
      return (
        <form className={css.tutorial}>
          ...
          <p>
            Depending on your distribution, you will need to run the following
            command:
          </p>
          <ul>
            <li>
          ...
          <p>
            For some desktop environments (e.g. Kubuntu or KDE), you may need to
            install an additional library for the communication with libsecret:
          </p>
          <ul>
            <li>
    ...
    if (error === "Found old data") {
      return (
        <form className={css.tutorial}>
          ...
          <p>
            Make sure you have a backup of your seeds and reset the wallet in
            Advanced settings.
          </p>
          <Button
            type="button"
