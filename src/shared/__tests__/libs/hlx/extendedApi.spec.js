    ...
    describe("when node runs an unsupported release", () => {
      ...
      it('should throw with an error "Node version not supported"', () => {
        return isNodeHealthy()
          ...
          .catch(error =>
            expect(error.message).to.equal("Node version not supported")
          );
      });
    });
              ...
              const resultMap = {
                getNodeInfo: {
                  ...
                  latestSolidSubtangleMilestoneIndex:
                    426550 - MAX_MILESTONE_FALLBEHIND,
                  ...
                  txs: [
                    head(
                      map(newZeroValueTransactionBytes, byteString => {
                        const transactionObject = asTransactionObject(
                          byteString
                        );
                        ...
                        return asTransactionStrings({
                          ...
                          timestamp: Math.round(
                            timestampLessThanAMinuteAgo / 1000
                          )
                        });
                      })
                    )
            ...
            .reply(200, (_, body) => {
              ...
              const resultMap = {
                getNodeInfo: {
                  ...
                  latestMilestoneIndex: 426550,
                  latestSolidSubtangleMilestoneIndex:
                    426550 - (MAX_MILESTONE_FALLBEHIND - 1),
                  latestMilestone: "c".repeat(64),
                  latestSolidSubtangleMilestone: "a".repeat(64)
                },
                getTransactionStrings: {
  ...
  describe("#allowsRemotePow", () => {
    describe('when has updated IRI version (version that has "features" prop in nodeInfo)', () => {
      describe('when has listed "RemotePOW" as a feature', () => {
        ...
        it("should return true", () => {
          return allowsRemotePow("http://localhost:14265").then(res =>
