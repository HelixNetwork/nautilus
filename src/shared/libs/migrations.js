  ...
  return reduce(
    ...
    (promise, accountName, idx) => {
      return promise.then(() => {
        ...
        const addressData = map(addresses, (data, address) =>
          assign({}, data, { address })
        );
          ...
          Account.createIfNotExists(
            ...
            assign({}, accountData, {
              ...
              index: !isUndefined(index)
                ? index
                : manuallyAssignAccountIndex(
                    get(thisAccountInfo, "accountIndex"),
                    idx
                  ),
              usedExistingSeed:
                get(setupInfo, `${accountName}.usedExistingSeed`) || false,
              displayedSnapshotTransitionGuide:
                get(tasks, `${accountName}.hasDisplayedTransitionGuide`) ||
                false,
              // Migrate failed transactions (Transaction that were never broadcasted from this wallet)
              transactions: [
                ...accountData.transactions,
                ...flatMap(
                  get(failedBundleHashes, `${accountName}`),
                  transactions =>
