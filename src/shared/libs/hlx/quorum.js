        ...
        case 'getNodeInfo:latestSolidRoundHash':
            return (...args) => {
                ...
                const preparedResults = prepare(
                    map(results, (result) => (isUndefined(result) ? undefined : [result.latestSolidRoundHash])),
    ...
    return {
        ...
        getLatestInclusion(hashes) {
            return isEmpty(hashes)
                ? Promise.resolve([])
                : findSyncedNodesIfNecessary().then((syncedNodes) =>
                      getQuorum(quorumSize)('getNodeInfo:latestSolidRoundHash', syncedNodes)
                          // If nodes cannot agree on the latestSolidRoundHash
                          // No need to proceed further.
                          .then(rejectIfEmptyHasTxBytes)
                          .then((latestSolidRoundHash) =>
                              getQuorum(quorumSize)('getInclusionStates', syncedNodes, hashes, [latestSolidRoundHash]),
                          ),
                  );
        },
        /**
