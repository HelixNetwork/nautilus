...
import { AccountsActionTypes, PollingActionTypes, TransfersActionTypes } from '../actions/types';
...
export const removeAccountAndReorderIndexes = (accountInfo, accountNameToDelete) => {
    ...
    return mapValues(
        ...
        (data) => ({
            ...data,
            index: data.index > index ? data.index - 1 : data.index,
        }),
    );
};
...
const updateAccountInfo = (state, payload) => ({
    accountInfo: {
        ...
        [payload.accountName]: {
            ...
            meta: payload.meta ||
                get(state.accountInfo, [payload.accountName, 'meta']) || {
                    type: 'keychain',
                },
