...
> **Note:** On initial sync Nautilus keeps a single unused address to be used as the receiving address.
...
On login, the application makes sure the account is synced, so on successful login promotion would be the first service that becomes active. Currently the market data is not enabled but will be available when helix token become listed in the exchanges in the near future. 
